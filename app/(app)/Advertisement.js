...
import { View, Text, TextInput, StyleSheet, ActivityIndicator, Alert, Pressable, Image, ScrollView, ToastAndroid } from 'react-native';
...
export default function Advertisement() {
  ...
  const checkFieldsFilled = () => {
    if (!selectedPlace || !selectedCategory || !selectedItem || !description || images.length === 0) {
      Alert.alert('Missing Field', 'Please fill in all fields and upload an image.');
      return false;
    }
    return true;
  };
  ...
  const uploadImage = async () => {
    try {
      ...
      setSelectedItem('');
      setDescription('');
      ToastAndroid.show('Adv added!', ToastAndroid.SHORT);
    } catch (error) {
...
<View style={{flexDirection:"row",justifyContent:"center",alignItems:"center",backgroundColor:"midnightblue",width:"90%",marginLeft:"5%",padding:0,borderRadius:45,marginBottom:20,marginTop:20,borderTopRightRadius:10,borderBottomLeftRadius:10,elevation:10}}>
      <LottieView style={{width:50,height:50}} source={require('../../util/Animation - 1716706724949.json')} autoPlay loop />
      <Text style={{ textAlign: "center", fontSize: 25, fontWeight: "bold", marginVertical: 15, color: "white" }}>Add Advertisement</Text></View>        
